...
### 8.2 任意規約: dispatch エッジ(フレームワーク規約の動的ディスパッチ、#43 / ADR-9)

`librarian resolve-dispatches` は、フレームワークが実行時に文字列/命名規約で解決する遷移
(呼び出し関係)を**規約に基づいて**束縛する。そのためにプラグインは、検出した遷移を新しい
エッジ種別 `dispatches` として、**規約を適用する前の生の値**で吐く:

| エッジ | `toName` | 意味 |
| --- | --- | --- |
| `dispatches` | `dispatch <controller>#<action>` | 遷移先を controller 値・action 値で名付ける(規約適用前) |

すべて `resolved = false` / `toId = null`。§8.1 の import binding と同型 — **参照点は事実だけを
名乗り、`<name>Controller` + public メソッドという規約の適用は後段(`resolve-dispatches`)が持つ**。
`<controller>` は runtime に見える生の値(CakePHP なら `redirect(['controller'=>'Foo'])` の `'Foo'`、
plugin/prefix 付きなら `Admin/Users` のように区切りごと)、`<action>` は同じく生のアクション名。
**リテラル文字列でないルーティング(変数・式)はそもそも吐かない**(静的解決不可能、ADR-9 スコープ外)。

**任意**である: 吐かないプラグインは dispatch 解決が起きないだけで degrade も偽エッジも発生しない
(resolve-dispatches はその言語のエッジに触れない)。**現状 PHP 抽出器のみ実装**(CakePHP の
`redirect` / `setAction`)。`dispatches` エッジ種別と `resolve-dispatches` の骨格は言語非依存なので、
他フレームワーク(Next.js ルーティング等)は抽出器側の検出と後段の規約を足せば同じ経路で届く
(ADR-9 Step 2)。数値と回帰は `docs/dispatch-baseline.md`。
