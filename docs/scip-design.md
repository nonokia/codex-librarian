...
1. **calls と references の区別が存在しない。** `SymbolRole` は Definition / Import /
   ...
   一次信号にしているため(`src/retrieval.ts`)、**エッジ種別の正をベース SCIP に置くことは
   できない**。
...
- protobuf バイナリ(`.scip` ファイル)の encode/decode は `src/scip.ts` の 1 箇所に閉じる
...
- **moniker ⇄ 既存 id の対応**: descriptors から file / container 連鎖 / name / kind が
  ...
  できる。この写像関数を `src/scip.ts` に 1 つだけ定義し、emit 側と ingest 側で共有する
  (非衝突性は既存 id スキームと同等)。
